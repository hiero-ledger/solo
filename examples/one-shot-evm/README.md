...
* **Deploys a complete EVM-ready network stack** with a consensus node, mirror node, and JSON-RPC relay in one command; the mirror-node explorer is opt-in (pass `--explorer mirror-node`)
...
2. **Deploy the network (default — no explorer):**
   ...
   * This will:
     ...
     * Deploy the EVM network (consensus node + mirror node + relay) using `solo one-shot single deploy --evm`
...
3. **Deploy variants:**
   ...
   |------|---------|-------------|
   | `task deploy` | `--evm` | Consensus node + mirror node + relay (no explorer) |
   | `task deploy-no-explorer` | `--evm --skip-explorer` | Same as default — explicitly skips explorer |
   | `task deploy-explorer-mirror-node` | `--evm --explorer mirror-node` | Adds mirror-node explorer |
