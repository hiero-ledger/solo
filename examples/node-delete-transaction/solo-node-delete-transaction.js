...
const logPrefix = 'SoloNodeDeleteTransaction:';
...
async function main() {
  ...
  try {
    const nodeDeleteTx = new NodeDeleteTransaction()
      .setNodeId(new Long(nodeIdFromNodeAlias('node1')))
      .freezeWith(nodeClient);
    const signedTx = await nodeDeleteTx.sign(transformedPrepareOutput.adminKey);
    const txResp = await signedTx.execute(nodeClient);
    const nodeDeleteReceipt = await txResp.getReceipt(nodeClient);
    console.log(`${logPrefix} NodeDeleteReceipt: ${nodeDeleteReceipt.toString()}`);
  } catch (error) {
