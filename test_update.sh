...
npm run solo-test -- block node add --deployment "${SOLO_DEPLOYMENT}" --chart-version v0.31.0-rc1
npm run solo-test -- keys consensus generate --gossip-keys --tls-keys --deployment "${SOLO_DEPLOYMENT}" -i "${NODE_ALIASES}"
npm run solo-test -- consensus network deploy --deployment "${SOLO_DEPLOYMENT}" -i "${NODE_ALIASES}" --rel
npm run solo-test -- consensus node setup --deployment "${SOLO_DEPLOYMENT}" -i "${NODE_ALIASES}" --release-tag v0.73.0-rc.1
#--local-build-path ../hiero-consensus-node/hedera-node/data
npm run solo-test -- consensus node start --deployment "${SOLO_DEPLOYMENT}" -i "${NODE_ALIASES}" --release-tag v0.73.0-rc.1
