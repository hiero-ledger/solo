...
# one-shot-full-delete-local-redeploy.sh – run a complete one-shot deploy to success,
# delete the local solo config (~/.solo), then verify that a subsequent re-deploy recovers
# cleanly by reconstructing local config from the remote Kubernetes ConfigMap and skipping
# steps whose components are already deployed.
#
# Usage:
#   bash .github/workflows/script/one-shot-full-delete-local-redeploy.sh
#
...
# ---------------------------------------------------------------------------
# Step 2 – delete local solo config (~/.solo); cluster remains running
# ---------------------------------------------------------------------------
banner "Step 2: deleting local solo config (~/.solo); cluster remains running"

log "Removing ~/.solo …"
rm -rf ~/.solo
log "Local config deleted"

# ---------------------------------------------------------------------------
# Step 3 – wait, then re-run the deploy (local config absent, cluster running)
# ---------------------------------------------------------------------------
banner "Step 3: waiting ${WAIT_SECONDS}s then redeploying (local config absent)"
