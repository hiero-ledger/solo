...
solo consensus node keys --gossip-keys --tls-keys -i node1,node2
solo deployment config create -i node1,node2 -n "${SOLO_NAMESPACE}" --context kind-"${SOLO_CLUSTER_NAME}" --email john@doe.com --deployment-clusters kind-"${SOLO_CLUSTER_NAME}" --cluster-ref kind-${SOLO_CLUSTER_NAME} --deployment "${SOLO_DEPLOYMENT}"
...
solo consensus node setup -i node1,node2 --deployment "${SOLO_DEPLOYMENT}" --release-tag "${CONSENSUS_NODE_VERSION}" -q
solo consensus node start -i node1,node2 --deployment "${SOLO_DEPLOYMENT}" -q
...
solo explorer node deploy -s "${SOLO_CLUSTER_SETUP_NAMESPACE}" --deployment "${SOLO_DEPLOYMENT}" --cluster-ref kind-${SOLO_CLUSTER_NAME}
...
npm run solo-test -- consensus node setup -i node1,node2 --deployment "${SOLO_DEPLOYMENT}" --release-tag "${CONSENSUS_NODE_VERSION}" -q
...
# Upgrade to v0.59.5
npm run solo-test -- consensus node upgrade -i node1,node2 --deployment "${SOLO_DEPLOYMENT}" --upgrade-version v0.59.5 -q
npm run solo-test -- ledger account create --deployment "${SOLO_DEPLOYMENT}" --hbar-amount 100

# Upgrade to latest version
export CONSENSUS_NODE_VERSION=$(grep 'HEDERA_PLATFORM_VERSION' version.ts | sed -E "s/.*'([^']+)';/\1/")
npm run solo-test -- consensus node upgrade -i node1,node2 --deployment "${SOLO_DEPLOYMENT}" --upgrade-version "${CONSENSUS_NODE_VERSION}" -q
npm run solo-test -- ledger account create --deployment "${SOLO_DEPLOYMENT}" --hbar-amount 100
...
npm run solo-test -- explorer node destroy --deployment "${SOLO_DEPLOYMENT}" --force
