...
# chart defaults still point at that blocked image and has no knowledge of the mirror image wired
# into this branch's version.ts. Inject it explicitly via --values-file so the source deployment
# doesn't fail before the migration itself is even exercised.
...
#   partially delivered to BN. CN v0.76.4 then resumes at N+1 instead of replaying N, leaving
