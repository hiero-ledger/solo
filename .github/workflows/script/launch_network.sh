...
solo network deploy --deployment "${SOLO_DEPLOYMENT}" "${CONSENSUS_NODE_FLAG[@]}"
solo node setup --deployment "${SOLO_DEPLOYMENT}" "${CONSENSUS_NODE_FLAG[@]}"
solo node start --deployment "${SOLO_DEPLOYMENT}"
solo mirror-node deploy --deployment "${SOLO_DEPLOYMENT}" --cluster-ref kind-${SOLO_CLUSTER_NAME} -q
...
solo mirror-node deploy  --deployment "${SOLO_DEPLOYMENT}" --enable-ingress --pinger
