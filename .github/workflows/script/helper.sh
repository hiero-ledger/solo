...
function create_test_account ()
{
  ...
  npm run solo-test -- ledger account create --deployment "${DEPLOYMENT_NAME}" --hbar-amount 10000 > test.log
  ...
  export SECOND_ACCOUNT_ID=$(grep \"accountId\" test.log | awk '{print $2}' | sed 's/"//g'| sed 's/,//g')
  echo "SECOND_ACCOUNT_ID=${SECOND_ACCOUNT_ID}"
  rm test.log
