...
# one-shot-interrupt-delete-local-redeploy.sh – interrupt a one-shot deploy after N seconds,
# delete the local solo config (~/.solo), then verify that a subsequent re-deploy recovers
# cleanly by reconstructing local config from the remote Kubernetes ConfigMap.
#
# Usage:
#   bash .github/workflows/script/one-shot-interrupt-delete-local-redeploy.sh [N_SECONDS]
#
...
# ---------------------------------------------------------------------------
# Step 2 – delete local solo config (~/.solo)
# ---------------------------------------------------------------------------
banner "Step 2: deleting local solo config (~/.solo)"
...
# ---------------------------------------------------------------------------
# Step 3 – wait, then re-run the deploy (should restore local config from remote)
# ---------------------------------------------------------------------------
