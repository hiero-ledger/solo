    ...
    while (attempt < maxAttempts) {
      try {
        ...
        const wrapperLines: string[] = [
          ...
          '',
          '# Handle partial initialization from a prior attempt that was interrupted mid-script ',
          '# (e.g. Postgres restarted while CREATE USER had already committed but CREATE DATABASE had not yet run).',
          `DB_EXISTS=$(psql -tc "SELECT 1 FROM pg_database WHERE datname = '${databaseName}'" 2>/dev/null | tr -d '[:space:]')`,
          'if [[ "${DB_EXISTS}" == "1" ]]; then',
          `  echo "Database '${databaseName}' already exists — initialization already complete, skipping."`,
          '  exit 0',
          'fi',
          '',
          '# If the owner role exists but the database does not, a prior run crashed between',
          '# CREATE USER and CREATE DATABASE. Drop the orphaned user so the init script can run cleanly.',
