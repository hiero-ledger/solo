    ...
    while (attempt < maxAttempts) {
      try {
        ...
        const wrapperLines: string[] = [
          ...
          '',
          '# The upstream init script grants CREATEROLE to the owner by default, but no longer',
          '# creates the REST API user unless CREATE_MIRROR_API_USER=true is set. Setting that',
          '# flag would skip the CREATEROLE grant that importer migrations still need, so create',
          '# the REST user separately after the default init path completes.',
          'psql --set ON_ERROR_STOP=1 \\',
          '  --set "restPassword=${REST_PASSWORD}" \\',
          '  --set "restUsername=${REST_USERNAME}" <<__SQL__',
