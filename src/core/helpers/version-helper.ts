...
export class VersionHelper {
   ...
   */
  public static async fetchLatestVersion(
    logger: SoloLogger,
    chartUrl: string,
    chartName: string,
  ): Promise<string> {
  ...
  private static async fetchLatestVersionFromRepo(
    ...
    const tempRepoName: string = `solo-temp-${crypto.randomBytes(8).toString('hex')}`;
    
    try {
      logger.debug(`Adding temporary Helm repo: ${tempRepoName} -> ${chartUrl}`);
      
      // Add the repository temporarily
      const addRepoCommand: string = `helm repo add ${tempRepoName} ${chartUrl} 2>&1`;
      execSync(addRepoCommand, {encoding: 'utf8', stdio: 'pipe'});
      
      // Update the repository
      const updateCommand: string = `helm repo update ${tempRepoName} 2>&1`;
      execSync(updateCommand, {encoding: 'utf8', stdio: 'pipe'});
      
      // Search for the chart and get all versions
      const searchCommand: string = `helm search repo ${tempRepoName}/${chartName} --versions -o json 2>&1`;
      ...
      try {
        const removeRepoCommand: string = `helm repo remove ${tempRepoName} 2>&1`;
        execSync(removeRepoCommand, {encoding: 'utf8', stdio: 'pipe'});
        logger.debug(`Removed temporary Helm repo: ${tempRepoName}`);
      } catch (cleanupError) {
        logger.warn(`Failed to remove temporary Helm repo ${tempRepoName}: ${cleanupError.message}`);
      }
    }
  }
