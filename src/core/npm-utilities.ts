...
export const unlinkLocalSoloPackages: (logger: SoloLogger) => Promise<void> = async function (
  logger: SoloLogger,
): Promise<void> {
  ...
  for (const item of listResult) {
    ...
    for (const packageName of matchesSoloPackages) {
      try {
        const logMessage: string = `Found earlier global installation of ${packageName}. Attempting to unlink...`;
        logger.showUser(logMessage);
