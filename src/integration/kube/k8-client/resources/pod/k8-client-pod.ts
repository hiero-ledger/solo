...
export class K8ClientPod implements Pod {
  ...
  public async portForward(
    ...
    try {
      ...
      await new ShellRunner().runExternalCommand(
        {
          commandPathOrName: cmd,
          commandArguments: cmdArguments,
      ...
      if (os.platform() === 'win32' && !isRetry && error?.message?.includes('listen EACCES')) {
        ...
        try {
          await new ShellRunner().runExternalCommand({
            commandPathOrName: 'net',
            commandArguments: ['stop', 'winnat'],
          });
        } catch (stopError) {
