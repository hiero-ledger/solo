  ...
  public prepareStagingDirectory(nodeAliasesProperty: string): SoloListrTask<AnyListrContext> {
    return {
      ...
      task: ({config}, task): SoloListr<AnyListrContext> => {
        ...
        const subTasks: SoloListrTask<AnyListrContext>[] = [
          {
            // After `network deploy` runs without --debug, the cached keys are removed from disk and live only in
            // the cluster secrets. Re-materialize any missing node keys from those secrets before staging them.
            // Nodes whose keys are already on disk (e.g. a node freshly generated by `node add`) are left untouched.
            title: 'Restore node keys from secrets if missing',
            task: async (): Promise<void> => {
              const consensusNodes: ConsensusNode[] = this.remoteConfig.getConsensusNodes();
              const nodesMissingKeys: ConsensusNode[] = consensusNodes.filter(
                (node: ConsensusNode): boolean =>
                  nodeAliases.includes(node.name as NodeAlias) &&
                  !fs.existsSync(
                    PathEx.join(config.keysDir, Templates.renderGossipPemPrivateKeyFile(node.name as NodeAlias)),
                  ),
              );
              if (nodesMissingKeys.length === 0) {
                return;
              }

              await this.platformInstaller.copyTLSKeysFromSecretToDirectory(
                this.remoteConfig.getContexts(),
                config.keysDir,
              );
              for (const node of nodesMissingKeys) {
                await this.platformInstaller.copyGossipKeysFromSecretToDirectory(node, config.keysDir);
              }
            },
          },
          {
