...
export class NodeCommandHandlers extends CommandHandler {
  ...
  private destroyExecuteTaskList(): SoloListrTask<NodeDestroyContext>[] {
    return [
      ...
      this.tasks.enableJvmDebugPortForwarding(),
  ...
  public async start(argv: ArgvStruct): Promise<boolean> {
    ...
    await this.commandAction(
      ...
      [
        ...
        this.tasks.startNodes('nodeAliases'),
        // a debugged node suspends its JVM until a debugger attaches through this forward, so it must exist before the ACTIVE check
        this.tasks.enableJvmDebugPortForwarding(),
        this.tasks.checkNodesAndProxiesAreActive('nodeAliases'),
        // the gRPC forward stays after the proxy checks so it never targets a proxy pod that startup recovery replaced
        this.tasks.enableGrpcPortForwarding(),
        this.tasks.waitForTss(),
