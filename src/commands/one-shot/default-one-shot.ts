...
export class DefaultOneShotCommand extends BaseCommand implements OneShotCommand {
      ...
      [
        ...
        },
        {
          ...
          task: async (): Promise<void> => {
            ...
            this.logger.showUser(
              `\nTo deploy, run:\n  ${CommandHelpers.soloCommand(FALCON_DEPLOY_COMMAND, CommandHelpers.optionFromFlag(flags.valuesFile), config.outputPath)}`,
            );
          },
        },
      ],
  ...
  public static generateFalconValuesYaml(config: FalconPrepareConfig): string {
    ...
    const header: string =
      ...
      `# Generated by: ${CommandHelpers.soloCommand(FALCON_PREPARE_COMMAND)}\n` +
      '# This file configures all components of the Hiero network deployment\n' +
      `#\n# Consensus nodes: ${config.numberOfConsensusNodes}\n` +
      '#\n# Usage:\n' +
      `#   ${CommandHelpers.soloCommand(FALCON_DEPLOY_COMMAND, optionFromFlag(flags.valuesFile), config.outputPath)}\n` +
      '#\n# To disable optional components, pass CLI flags:\n' +
      `#   ${CommandHelpers.negatedOptionFromFlag(flags.deployMirrorNode)}\n` +
      `#   ${CommandHelpers.negatedOptionFromFlag(flags.deployExplorer)}\n` +
      `#   ${CommandHelpers.negatedOptionFromFlag(flags.deployRelay)}\n\n`;
