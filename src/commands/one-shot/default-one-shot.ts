...
export class DefaultOneShotCommand extends BaseCommand implements OneShotCommand {
      ...
      [
        ...
        },
        {
          ...
          task: async (): Promise<void> => {
            ...
            this.logger.showUser(
              `\nTo deploy, run:\n  ${soloCommand(OneShotCommandDefinition.FALCON_DEPLOY_COMMAND, optionFromFlag(flags.valuesFile), config.outputPath)}`,
            );
          },
        },
      ],
  ...
  public static generateFalconValuesYaml(config: FalconPrepareConfig): string {
    ...
    const header: string =
      ...
      `# Generated by: ${soloCommand(OneShotCommandDefinition.FALCON_PREPARE_COMMAND)}\n` +
      '# This file configures all components of the Hiero network deployment\n' +
      `#\n# Consensus nodes: ${config.numberOfConsensusNodes}\n` +
      '#\n# Usage:\n' +
      `#   ${soloCommand(OneShotCommandDefinition.FALCON_DEPLOY_COMMAND, optionFromFlag(flags.valuesFile), '<path-to-this-file>')}\n` +
