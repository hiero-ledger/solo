      ...
      [
        {
          ...
          task: async (ctx, task) => {
            ...
            const outputDir = argv[flags.outputDir.name] || constants.SOLO_LOGS_DIR;
            ...
            const debugDir = path.join(outputDir, `solo-debug-${deployment}-${timestamp}`);
            const zipFile = `${debugDir}.zip`;
            ...
            if (!fs.existsSync(ctx.config.debugDir)) {
              fs.mkdirSync(ctx.config.debugDir, {recursive: true});
            }

            self.logger.showUser(chalk.cyan(`Collecting debug information to: ${ctx.config.debugDir}`));
          },
        },
        {
          title: 'Collect cluster information',
          task: async (ctx, task) => {
            const clusterInfoDir = path.join(ctx.config.debugDir, 'cluster-info');
            if (!fs.existsSync(clusterInfoDir)) {
              fs.mkdirSync(clusterInfoDir, {recursive: true});
            }

            try {
              // Get current context
              const contexts = await self.run('kubectl config get-contexts -o name');
              fs.writeFileSync(path.join(clusterInfoDir, 'contexts.txt'), contexts.join('\n'));

              // Get current context details
              const currentContext = await self.run('kubectl config current-context');
              fs.writeFileSync(path.join(clusterInfoDir, 'current-context.txt'), currentContext.join('\n'));

              // Get cluster info
              const clusterInfo = await self.run('kubectl cluster-info dump --output-directory -');
              fs.writeFileSync(path.join(clusterInfoDir, 'cluster-info.txt'), clusterInfo.join('\n'));

              // Get nodes
              const nodes = await self.run('kubectl get nodes -o wide');
              fs.writeFileSync(path.join(clusterInfoDir, 'nodes.txt'), nodes.join('\n'));

              // Get all namespaces
              const namespaces = await self.run('kubectl get namespaces');
              fs.writeFileSync(path.join(clusterInfoDir, 'namespaces.txt'), namespaces.join('\n'));
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            const helmDir = path.join(ctx.config.debugDir, 'helm-releases');
            ...
            try {
              ...
              fs.writeFileSync(path.join(helmDir, 'all-releases.yaml'), releases.join('\n'));

              // Get helm releases in specific namespace
              const namespaceReleases = await self.run(
                `helm list -n ${ctx.config.namespace.toString()} -o yaml`,
              );
              fs.writeFileSync(path.join(helmDir, `${ctx.config.namespace.toString()}-releases.yaml`), namespaceReleases.join('\n'));
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            const resourcesDir = path.join(ctx.config.debugDir, 'resources');
            ...
            try {
              ...
              fs.writeFileSync(path.join(resourcesDir, 'pods.yaml'), yaml.stringify(podsData));

              // Get pod status details
              const podStatus = await self.run(`kubectl get pods -n ${namespace.toString()} -o wide`);
              fs.writeFileSync(path.join(resourcesDir, 'pod-status.txt'), podStatus.join('\n'));

              // Get services
              const services = await self.run(`kubectl get services -n ${namespace.toString()} -o yaml`);
              fs.writeFileSync(path.join(resourcesDir, 'services.yaml'), services.join('\n'));

              // Get deployments
              const deployments = await self.run(`kubectl get deployments -n ${namespace.toString()} -o yaml`);
              fs.writeFileSync(path.join(resourcesDir, 'deployments.yaml'), deployments.join('\n'));

              // Get statefulsets
              const statefulsets = await self.run(`kubectl get statefulsets -n ${namespace.toString()} -o yaml`);
              fs.writeFileSync(path.join(resourcesDir, 'statefulsets.yaml'), statefulsets.join('\n'));
              ...
              fs.writeFileSync(path.join(resourcesDir, 'configmaps.yaml'), yaml.stringify(configMapsData));
              ...
              fs.writeFileSync(path.join(resourcesDir, 'secrets.json'), JSON.stringify(secretNames, null, 2));

              // Get persistent volume claims
              const pvcs = await self.run(`kubectl get pvc -n ${namespace.toString()} -o yaml`);
              fs.writeFileSync(path.join(resourcesDir, 'persistent-volume-claims.yaml'), pvcs.join('\n'));

              // Get ingresses
              const ingresses = await self.run(`kubectl get ingress -n ${namespace.toString()} -o yaml`);
              fs.writeFileSync(path.join(resourcesDir, 'ingresses.yaml'), ingresses.join('\n'));
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            const logsDir = path.join(ctx.config.debugDir, 'pod-logs');
            ...
            try {
              ...
              for (const pod of pods) {
                ...
                const logFile = path.join(logsDir, `${podName}.log`);
                ...
                try {
                  ...
                  try {
                    const logs = await self.run(
                      `kubectl logs ${podName} -n ${namespace.toString()} --timestamps=true`,
                    );
                    fs.writeFileSync(logFile, logs.join('\n'));
                  } catch (innerError) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            try {
              ...
              const networkPods: Pod[] = await k8
                .pods()
                .list(namespace, ['solo.hedera.com/type=network-node']);

              if (networkPods.length > 0) {
                const nodeLogsDir = path.join(ctx.config.debugDir, 'network-node-logs');
                await self.networkNodes.getLogs(namespace, undefined, nodeLogsDir);
              }
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            try {
              ...
              fs.writeFileSync(path.join(ctx.config.debugDir, 'port-forwards.txt'), portForwards.join('\n'));
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            const systemDir = path.join(ctx.config.debugDir, 'system-info');
            ...
            try {
              ...
              fs.writeFileSync(path.join(systemDir, 'solo-version.txt'), soloVersion.join('\n'));

              // Get kubectl version
              const kubectlVersion = await self.run('kubectl version --short --client || kubectl version');
              fs.writeFileSync(path.join(systemDir, 'kubectl-version.txt'), kubectlVersion.join('\n'));

              // Get helm version
              const helmVersion = await self.run('helm version --short');
              fs.writeFileSync(path.join(systemDir, 'helm-version.txt'), helmVersion.join('\n'));

              // Get docker/podman info
              const containerInfo = await self.run('docker info 2>/dev/null || podman info 2>/dev/null || echo "No container runtime found"');
              fs.writeFileSync(path.join(systemDir, 'container-runtime.txt'), containerInfo.join('\n'));

              // Get kind clusters
              const kindClusters = await self.run('kind get clusters || echo "No kind clusters found"');
              fs.writeFileSync(path.join(systemDir, 'kind-clusters.txt'), kindClusters.join('\n'));
            } catch (error) {
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            await self.zippyService.zip(ctx.config.debugDir, ctx.config.zipFile);
            self.logger.showUser(chalk.green(`\n✓ Debug information collected successfully!`));
            self.logger.showUser(chalk.cyan(`  Directory: ${ctx.config.debugDir}`));
            self.logger.showUser(chalk.cyan(`  Archive: ${ctx.config.zipFile}`));
          },
        },
      ],
      {
