...
const logActiveHandles = (logger: SoloLogger | undefined): void => {
  ...
  const payload = {
    activeHandles: handles.map(handle => describe(handle)),
    activeRequests: requests.map(request => describe(request)),
  };
  const detailedHandles = handles
    ...
    .filter(
      entry =>
        entry.summary.type === 'MessagePort' ||
        entry.summary.type === 'ChildProcess' ||
        entry.summary.type === 'TLSSocket',
    );
  ...
  if (logger) {
    ...
    if (asyncHandleStacks.size > 0) {
      logger.showUser(`Async handle init stacks: ${JSON.stringify([...asyncHandleStacks.values()])}`);
    }
  } else {
    ...
    if (asyncHandleStacks.size > 0) {
      console.warn(`Async handle init stacks: ${JSON.stringify([...asyncHandleStacks.values()])}`);
    }
  }
};
