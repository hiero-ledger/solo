...
> **Note**: The `--deployment` flag must **not** be used inside a values file for the `solo one-shot falcon` command. The deployment name is auto-generated by the command and cannot be overridden through the values file. Specifying `--deployment` in the values file will cause the deployment to fail.
