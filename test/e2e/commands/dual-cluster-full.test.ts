...
async function verifyExplorerDeployWasSuccessful(
  ...
  const k8Factory: K8Factory = container.resolve<K8Factory>(InjectTokens.K8Factory);
  const k8: K8 = k8Factory.getK8(contexts[1]);
  const hieroExplorerPods: Pod[] = await k8
    .pods()
    .list(namespace, [
  ...
  expect(hieroExplorerPods).to.have.lengthOf(1);
  let portForwarder: ExtendedNetServer;
  try {
    portForwarder = await k8.pods().readByReference(hieroExplorerPods[0].podReference).portForward(8080, 8080);
    await sleep(Duration.ofSeconds(2));
