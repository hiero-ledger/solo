  ...
  .withTestSuiteCallback((endToEndTestSuite: EndToEndTestSuite): void => {
    describe('Quick Start E2E Test', (): void => {
      ...
      it(`${testName}: solo cluster-ref connect`, async (): Promise<void> => {
        ...
        expect(clusterReferences.get(endToEndTestSuite.clusterReferenceNameArray[0])?.toString()).to.equal(
          endToEndTestSuite.contexts[0],
        );
        expect(clusterReferences.get(endToEndTestSuite.clusterReferenceNameArray[1])?.toString()).to.equal(
      ...
      it(`${testName}: solo deployment add-cluster`, async (): Promise<void> => {
        ...
        expect(consensusNodes[0].metadata.cluster).to.equal(endToEndTestSuite.clusterReferenceNameArray[0]);
        expect(consensusNodes[1].metadata.cluster).to.equal(endToEndTestSuite.clusterReferenceNameArray[1]);
