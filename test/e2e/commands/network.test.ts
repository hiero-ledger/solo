...
describe('NetworkCommand', function networkCommand(): void {
  this.bail(true);
  ...
  it('network deploy command should succeed', async () => {
    ...
    const pvcs = await bootstrap.opts.k8Factory.default().pvcs().list(namespace, []);
    bootstrap.opts.logger.showList('PVCs', pvcs);
  }).timeout(Duration.ofMinutes(4).toMillis());

  it('application env file contents should be in cached values file', () => {
    // @ts-expect-error - TS2341: to access private property
    const valuesYaml = fs.readFileSync(Object.values(bootstrap.cmd.networkCmd.profileValuesFile)[0]).toString();
  ...
  it('network destroy should success', async () => {
    ...
    try {
      ...
      while ((await bootstrap.opts.k8Factory.default().pods().list(namespace, ['app=minio'])).length > 0) {
        bootstrap.opts.logger.showUser('Waiting for minio container to be deleted...');
        await sleep(Duration.ofSeconds(3));
      }
      ...
      await expect(bootstrap.opts.k8Factory.default().pvcs().list(namespace, [])).eventually.to.have.lengthOf(0);

      // check if secrets are deleted
      await expect(bootstrap.opts.k8Factory.default().secrets().list(namespace)).eventually.to.have.lengthOf(0);
    } catch (error) {
      bootstrap.opts.logger.showUserError(error);
      expect.fail();
    }
