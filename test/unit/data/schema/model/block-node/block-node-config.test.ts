...
describe('BlockNodeConfig', () => {
  const schema: BlockNodeConfigSchemaDefinition = new BlockNodeConfigSchemaDefinition(
    new ClassToObjectMapper(ConfigKeyFormatter.instance()),
  );
  const blockNodeVersion: string = '0-36-0';
  const blockNodeConfigPath = `test/data/v${blockNodeVersion}-block-node-config.yaml`;
  ...
  describe('Class Transformer', () => {
    ...
    beforeEach(() => {
      yamlData = readFileSync(blockNodeConfigPath, 'utf8');
    ...
    it('should transform plain to class', async () => {
      const bnc = await schema.transform(plainObject);
      expect(bnc).to.not.be.undefined.and.to.not.be.null;
      expect(bnc).to.be.instanceOf(BlockNodeConfigSchema);
      expect(typeof bnc.schemaVersion).to.eq('number');

      // Verify helmChart properties
      expect(bnc.helmChart).to.be.instanceOf(HelmChartSchema);
      expect(bnc.helmChart.name).to.eq('block-node-deployment');
      expect(bnc.helmChart.release).to.eq('block-node-deployment');
      expect(bnc.helmChart.repository).to.eq('oci://ghcr.io/hashgraph/solo-charts');
    });
